...
#
# However, when we do this, we also import `nssm/__init__.py'. If this
...
def get_project_files():
    ...
    for top, subdirs, files in os.walk('.'):
        ...
        for f in files:
            if f.startswith('.'):
                continue
            project_files.append(os.path.join(top, f).decode('ascii'))
...
def _test(args):
    ...
    return pytest.main(PYTEST_FLAGS + [TESTS_DIRECTORY] + args)
...
def convert_md_to_rst(filename):
    ...
    try:
        import pypandoc
        output = pypandoc.convert_text(read(filename), 'rst', format='md')
        output = output.replace('\r', '')
    except ImportError:
        errmsg = 'Could not convert "{}" to reStructuredText format'
        print_failure_message(errmsg.format(filename))
        output = read(filename)
    return output


def get_package_dependencies():
    """Read the requirements.txt file and parse into a list of requirements.
    """

    requirements = []
    with open('requirements.txt', 'r') as req_file:
        requirements.extend(req_file.readlines())

    requirements.extend(python_version_specific_requires)
